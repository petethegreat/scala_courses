...
object WikipediaRanking extends WikipediaRankingInterface {
   ...
   */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.filter(x => x.mentionsLanguage(lang)).count.toInt
   ...
   *   several seconds.
   */
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = langs.map(x=> (x,occurrencesOfLang(x,rdd))).sortBy(_._2)(Ordering[Int].reverse)
