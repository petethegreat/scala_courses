...
When creating the tasks, the current computation (presumably the main thread) continues. The join calls are \textem{blocking} - the main thread above will wait until task t1 concludes. Subsequent calls to t1.join quickly return the same result.
...
D(e) - the depth of a parallel program - the number of steps it iwould be broken into given unbounded parallelism.
